# Liva Style Crafts — Storefront

A from-scratch replacement for the livastyle.in Shopify store: Next.js
...
- **Supabase** — Postgres database, email/password + Google login, Storage for product images
...
3. In the **SQL Editor**, run the migration files in order:
   ...
   - `supabase/migrations/0004_discounts_and_analytics.sql`
   (0002 also creates a public Storage bucket named `products` — check
   **Storage** in the dashboard to confirm it exists.)

## 2. Auth: email/password + Google, with Resend for auth emails

Sign-in supports both Supabase's built-in email/password auth (`/login`,
`/signup`, `/forgot-password`, `/reset-password`) and **Sign in with
Google**, side by side.

### 2a. Email/password auth emails via Resend
...
2. Add an **Authorized redirect URI**. Supabase gives you the exact URL in
...
4. In Supabase, go to **Authentication → Providers → Google**, toggle it on,
   and paste in the Client ID and Client secret. This is also where you'll
   find the exact redirect URI to use in step 2.
5. No app-side env vars are needed for Google — the app calls
   `supabase.auth.signInWithOAuth({ provider: "google" })` and Supabase
   handles the rest via `app/auth/callback/route.ts`.

### 2c. URL configuration (needed for both)
...
- Add these to **Redirect URLs** (and your production equivalents later):
  - `http://localhost:3000/auth/confirm` (email confirmation / password reset)
  - `http://localhost:3000/auth/callback` (Google sign-in)
...
3. You'll add the webhook (**Settings → Webhooks**) *after* deploying, once
   you have a public URL — see step 8.
...
live `livastyle.in` Shopify JSON endpoints and loads them into your new
...
Visit http://localhost:3000. Create an account once (via "Sign in" — either
Google or "Sign up" with email), then make yourself an admin by running this
...
3. Add all the same environment variables from `.env.local` in the Vercel
   ...
   domain (e.g. `https://livastyle.in` or the `*.vercel.app` URL).
4. Deploy.

### After deploying

- **Supabase → Authentication → URL Configuration**: set **Site URL** to
  your production domain, and add `https://YOUR-DOMAIN/auth/confirm` and
  `https://YOUR-DOMAIN/auth/callback` to **Redirect URLs**.
- **Google Cloud Console → Credentials**: no change needed — the redirect
  URI you added points at Supabase, not at your app domain.
...
app/                      Pages & API routes (Next.js App Router)
  admin/                  Admin panel (products, collections, orders, discounts, homepage content)
  api/checkout/           Razorpay order creation, discount validation + payment verification
  ...
  login/, signup/,        Email/password + Google auth pages
  forgot-password/,
  reset-password/
components/               Shared UI + admin form components (incl. GoogleSignInButton)
lib/                      Supabase clients, Razorpay/Resend helpers, cart store, types
emails/                   React Email templates sent via Resend
supabase/migrations/      SQL schema, RLS policies, storage bucket, stock functions, discounts
scripts/seed.ts           One-time Shopify → Supabase catalog migration (source: livastyle.in)
```

## What's editable from the admin panel (`/admin`)

- **Products** — create/edit/delete, price, stock, images, variants, which
  collection they belong to, active/featured flags.
- **Collections** — create/edit/delete, cover image, description, sort order.
- **Discount codes** — percent/fixed, active window.
- **Orders** — view order details and update status (pending → paid →
  shipped → delivered, etc.), export CSV.
